...
if execute_btn:
    ...
    with st.spinner('🚀 데이터 복구 및 시세 분석 중...'):
        current_stock_assets = 0; total_today_profit = 0; total_prev_asset = input_cash; stock_data_cache = []
        cat_cur = {"US": 0, "KR": 0, "ETF": 0}; cat_prev = {"US": 0, "KR": 0, "ETF": 0}

        # 1. 삼성전자 계산
        sam_price, sam_change, sam_prev = get_real_price_and_change(SAMSUNG_TICKER, "KR")
        sam_amt = sam_price * SAMSUNG_QTY; sam_prev_amt = sam_prev * SAMSUNG_QTY
        current_stock_assets += sam_amt; total_today_profit += (sam_amt - sam_prev_amt); total_prev_asset += sam_prev_amt
        cat_cur["KR"] += sam_amt; cat_prev["KR"] += sam_prev_amt
        ...
        for i, p in enumerate(all_stocks):
            ...
            prev_amt = user_holdings[i] * prev_c * rate
            
            current_stock_assets += my_amt; total_today_profit += (my_amt - prev_amt); total_prev_asset += prev_amt
            cat_cur[p['type']] += my_amt; cat_prev[p['type']] += prev_amt
            stock_data_cache.append({"price_krw": price*rate, "my_amt": my_amt, "cp": cp, "profit": my_amt-prev_amt, "type": p['type'], "ratio": p['ratio']})
        ...
        st.session_state.update({
            ...
            "total_profit": total_today_profit, "total_cp": (total_today_profit/total_prev_asset*100) if total_prev_asset>0 else 0,
            "sam_price": sam_price, "sam_cp": sam_change, "sam_profit": sam_amt-sam_prev_amt,
            "cache": stock_data_cache, "user_holdings": user_holdings, "input_cash": input_cash,
            "cat_stats": {"US": [cat_cur["US"], cat_prev["US"]], "KR": [cat_cur["KR"], cat_prev["KR"]], "ETF": [cat_cur["ETF"], cat_prev["ETF"]]},
...
if st.session_state.analyzed:
    ...
    for i, p in enumerate(all_stocks):
        c = st.session_state.cache[i]; my_qty = st.session_state.user_holdings[i]
        ...
        rows.append({"종목": get_brand(p['name'])["name"], "현재가(₩)": f"{c['price_krw']:,.0f}원", "등락률": f"{c['cp']:+.2f}%", "오늘수익": f"{c['profit']:+,.0f}원", "실제비중": f"{(c['my_amt']/st.session_state.total_asset):.1%}", "실제금액": f"{c['my_amt']:,.0f}원", "실행": action, "오늘수익숫자": c['profit'], "실제금액숫자": c['my_amt']})
    ...
    # --- 요약표 ---
    st.write("---")
    st.subheader("📋 포트폴리오 요약 (해외➔국내➔현금성➔예수금➔총합)")
    sum_rows = []
    ...
    sum_rows.append({"종목":"📊 포트폴리오 총합", "등락률":f"{st.session_state.total_cp:+.2f}%", "오늘수익":f"{st.session_state.total_profit:+,.0f}원", "실제비중":"100.0%", "실제금액":f"{st.session_state.total_asset:,.0f}원"})
    ...
    def add_pro_lines(fig, ohlc_df, title):
    ...
    with tab1:
        ...
        st.plotly_chart(add_pro_lines(go.Figure(data=[go.Candlestick(x=w5.index, open=w5['Open'], high=w5['High'], low=w5['Low'], close=w5['Close'])]), w5, "전체"), use_container_width=True)
    with tab2:
        kr_tkrs = [SAMSUNG_TICKER] + [p['ticker'] for p in all_stocks if p['country']=='KR']
        w_kr = calc_ohlc(st.session_state.df_hist, kr_tkrs)
        st.plotly_chart(add_pro_lines(go.Figure(data=[go.Candlestick(x=w_kr.index, open=w_kr['Open'], high=w_kr['High'], low=w_kr['Low'], close=w_kr['Close'])]), w_kr, "국장"), use_container_width=True)
    with tab3:
        us_tkrs = [p['ticker'] for p in all_stocks if p['country']=='US']
        w_us = calc_ohlc(st.session_state.df_hist, us_tkrs)
        st.plotly_chart(add_pro_lines(go.Figure(data=[go.Candlestick(x=w_us.index, open=w_us['Open'], high=w_us['High'], low=w_us['Low'], close=w_us['Close'])]), w_us, "미장"), use_container_width=True)
    with tab4:
        ...
        st.plotly_chart(add_pro_lines(fa, calc_ohlc(dfh, all_tickers[:-1]), "전체"), use_container_width=True)
