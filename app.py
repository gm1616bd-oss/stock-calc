...
if execute_btn:
    ...
    with st.spinner('실시간 시세 및 3년치 글로벌 차트 로딩 중... (약 15초 소요)'):
        ...
        for i, p in enumerate(all_stocks):
            ...
            # 카테고리 분류용 임시 키 (invest_portfolio 구조에 맞춰 설정)
            # 여기서는 편의상 처음 5개 종목을 ETF로 분류
            if i < 5:
                p_type = "ETF"
            elif p['country'] == 'US':
                p_type = "US"
        ...
        df_hist = yf.download(tickers, period="3y", threads=True, progress=False)
        df_5m = yf.download(tickers, period="2d", interval="5m", threads=True, progress=False)
...
if st.session_state.analyzed:
    ...
    stock_rows.append({
        ...
        "목표수량": "-", "내보유": str(SAMSUNG_QTY), "실행": "🔒 고정",
    ...
    for i, p in enumerate(all_stocks):
        ...
        if i < 5: 
            target_amt = st.session_state.rebalance_budget * cached['ratio']
            display_target_ratio = f"{cached['ratio']:.1%}"
        else: 
            target_amt = budget_invest * cached['ratio']
            display_target_ratio = f"{(0.63 * cached['ratio']):.1%}"
    ...
    for code, label in [("US", "🌎 해외주식 총합"), ("KR", "🇰🇷 국내주식 총합"), ("ETF", "🛡️ 현금성ETF 총합")]:
        ...
        c_cur, c_prev, c_prev2 = vals[0], vals[1], vals[2]
    ...
    with col_chart:
        ...
        try:
            ...
            ath_val = hist_H.max(); ath_d = hist_H.idxmax()
            low_3m = hist_L.tail(60).min(); low_d = hist_L.tail(60).idxmin()
            curr = st.session_state.total_asset
            last_date = df_hist.index[-1]
            zoom_start = last_date - pd.Timedelta(days=90)

            # 수직 구분선 데이터
            f_m = [g.index[0] for _, g in df_hist.groupby([df_hist.index.year, df_hist.index.month])]
            f_y = [g.index[0] for _, g in df_hist.groupby(df_hist.index.year)]

            with tab1:
                fig_candle = go.Figure(data=[go.Candlestick(x=hist_C.index, open=hist_O, high=hist_H, low=hist_L, close=hist_C)])
                fig_candle.add_hline(y=ath_val, line_dash="dash", line_color="gray", opacity=0.5)
                fig_candle.add_hline(y=low_3m, line_dash="dash", line_color="gray", opacity=0.5)
                fig_candle.add_hline(y=curr, line_dash="dot", line_color="red", opacity=0.8)
                fig_candle.add_annotation(x=ath_d, y=ath_val, text=f"🚩 전고점", showarrow=True)
                for d in f_m:
                    if d in f_y: fig_candle.add_vline(x=d, line_dash="solid", line_color="black", line_width=2)
                    else: fig_candle.add_vline(x=d, line_dash="dot", line_color="rgba(150,150,150,0.5)")
                fig_candle.update_layout(xaxis_range=[zoom_start, last_date], yaxis_tickformat=",.0f", height=500, margin=dict(l=0,r=0,t=30,b=0))
                st.plotly_chart(fig_candle, use_container_width=True)

            with tab2:
                s_ca = pd.Series(st.session_state.input_cash, index=df_hist.index)
                s_et = pd.Series(0, index=df_hist.index)
                s_u = pd.Series(0, index=df_hist.index)
                s_k = pd.Series(0, index=df_hist.index)
                s_k += df_hist['Close'][SAMSUNG_TICKER].ffill().bfill() * SAMSUNG_QTY
                for i, p in enumerate(all_stocks):
                    q = st.session_state.user_holdings[i]
                    if q > 0:
                        val = df_hist['Close'][p['ticker']].ffill().bfill() * q * (st.session_state.exc_rate if p['country']=='US' else 1)
                        if i < 5: s_et += val
                ...
                fa.update_layout(xaxis_range=[zoom_start, last_date], yaxis_tickformat=",.0f", height=500, margin=dict(l=0,r=0,t=30,b=0))
    ...
    with col_pie:
        ...
        try:
            pie_data = [{"종목": get_brand("삼성전자")["name"], "금액": st.session_state.sam_amt}]
            for i, p in enumerate(all_stocks):
            ...
            fig_p = px.pie(df_p, values='금액', names='종목', hole=0.4, color='종목', color_discrete_map={v["name"]: v["color"] for v in brand_meta.values()})
            fig_p.update_layout(height=530, showlegend=False, margin=dict(l=0,r=0,t=30,b=0))
            st.plotly_chart(fig_p, use_container_width=True)
        except: st.warning("비율 데이터를 표시할 수 없습니다.")
