...
with h4: 
    exc_rate = yf.Ticker("KRW=X").fast_info.get('last_price', 1420.0)
    st.info(f"**💵 환율**\n### {exc_rate:,.1f}원")
...
if execute_btn:
    ...
    with st.spinner('🚀 시세 분석 중...'):
        # 1. 시세 데이터 벌크 다운로드 (실시간성 확보를 위해 period=2d)
        ...
        def process_stock(ticker, qty, country, p_type, ratio):
            # 전일 종가와 현재가 추출
            prices = df_close[ticker].ffill().dropna()
            if len(prices) >= 2:
                p_curr = prices.iloc[-1]
                p_prev = prices.iloc[-2]
            else:
                p_curr = prices.iloc[-1] if not prices.empty else 0
                p_prev = p_curr # 데이터 부족 시 등락 0처리
            
            rate = exc_rate if country == "US" else 1
            cur_val = qty * p_curr * rate
            pre_val = qty * p_prev * rate
            
            cp = ((p_curr - p_prev) / p_prev * 100) if p_prev > 0 else 0
            profit = cur_val - pre_val
            
            return p_curr * rate, cp, profit, cur_val, pre_val

        # 삼성전자
        sp_krw, scp, sprof, samt, spamt = process_stock(SAMSUNG_TICKER, SAMSUNG_QTY, "KR", "KR", 0)
        curr_assets += samt; t_profit += sprof; t_prev_a += spamt
        cat_c["KR"] += samt; cat_p1["KR"] += spamt

        # 15종목
        for i, p in enumerate(all_stocks):
            p_krw, cp, prof, samt, spamt = process_stock(p['ticker'], user_h[i], p['country'], p['type'], p['ratio'])
            curr_assets += samt; t_profit += prof; t_prev_a += spamt
            cat_c[p['type']] += samt; cat_p1[p['type']] += spamt
            cache.append({"price_krw": p_krw, "my_amt": samt, "cp": cp, "profit": prof, "type": p['type'], "ticker": p['ticker'], "ratio": p['ratio']})
        ...
        st.session_state.update({
            "total_asset": total_asset, "rebalance_budget": total_asset - samt,
            "t_profit": t_profit, "t_cp": (t_profit/t_prev_a*100) if t_prev_a > 0 else 0,
            "sam_amt": samt, "sam_price": sp_krw/1, "sam_cp": scp, "sam_profit": sprof,
...
if st.session_state.analyzed:
    ...
    summary_row = pd.DataFrame([{"종목": "✨ 선택 분류 합계", "오늘수익": f"{f_sum_p:+,.0f}원", "실제비중": f"{(f_sum_a/st.session_state.total_asset):.1%}", "실제금액": f"{f_sum_a:,.0f}원", "오늘수익숫자": f_sum_p}])
    ...
    def add_lines(fig, ohlc_df, title):
        ...
        fig.update_yaxes(tickformat=",.0f", range=[max(0, low_3m*0.9), ath*1.15])
        return fig
